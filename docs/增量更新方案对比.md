...
> **历史背景分支：** `feature/multi-worker-parallel`（方案讨论时）  
> **编写日期：** 2026-06-22  
> **已选方案：** **方案 B（扫描快照 + 增量处理）**，已落地并继承至当前推荐分支 **`feature/arch-data-dir-cleanup`**；每周全量校准见 `FULL_SCAN_CALIBRATION_DAYS`。
> **现行操作：** [QUICK_START.md](QUICK_START.md) · [CONSOLE.md](CONSOLE.md)
...
| 全局去重 | `state/shared_state.py` |
| **扫描快照（方案 B v1）** | `state/scan_snapshot.py` → `ScanSnapshot` / `scan_snapshot.db` |
| 全量扫描 | `wiki_scanner.py` → `scan_space` |
| 扫描缓存 | `wiki_scan_cache.db`（`USE_CACHE`） |
| 已复制后源正文变更 | `tools/refresh_target_from_source.py`（**内容**单向刷新，与扫描增量无关） |
...
> **与「源→TARGET 内容刷新」的区分：** 扫描快照解决的是「源目录**新出现**的叶子要不要再分类复制」；`refresh_target_from_source` 解决的是「**已经复制过**的文档，源侧又改了正文，如何按需更新 TARGET」。二者互补，不做双向同步。
