...
| 变量 | 说明 |
|------|------|
| `WORKER_ID` | 你的标识，须与 `scan_folders.json` 里 `assignee` **完全一致** |
...
| 文档元数据表 | **[扫源] 按 token 分表** / 指定 folder | 多人并行常用；扫源清单 |
| 文档元数据表 | 试跑 20 篇 | 不写表 |
...
4. 填 `assignee`（须与执行人 `.env` 的 `WORKER_ID` 一致）  
5. 点 **添加并保存** → 立刻写入清单文件  
...
- `enabled`：是否参与跑  
- `assignee`：负责人（须与其 `WORKER_ID` 一致）  
- `priority`：排序  
- `token` / `name` / `id`：一般不要乱改 token  

改完点 **保存清单**。行末「移除」只从表格删掉，需再保存才落盘。

一人跑全部增量时：把各条目 `assignee` 设成同一 `WORKER_ID`，然后用「【增量更新】分类复制 · 我的文件夹」。先点「运行 → 列出清单分工」，确认名下带 `*` 的条目无误。
...
1. 多人并行时每人只跑：**元数据 → 按 token 分表**（不要同时写同一张汇总表）  
2. 全部完成后，再跑：**元数据 → 仅汇总表**  
3. 在飞书 `TARGET_PARENT_TOKEN` 下查看分表与汇总表  
