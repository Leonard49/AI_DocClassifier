...
左侧按分组选任务，右侧看实时输出。筛选：`全部 | 主流程 | 副本增强 | 文档元数据表 | 归纳新标题 | 运维纠偏`。

| 筛选 | 典型任务 | 作用 |
|------|----------|------|
| 主流程 | 列出清单分工 | 确认 assignee / 自己名下文件夹 |
| 主流程 | 分类复制（我的文件夹） | `main.py --all-assigned` |
| 主流程 | 分类复制（清单全部 enabled） | **慎用**，会跑所有 enabled |
| 主流程 | 分类复制（指定 folder id） | 先在下方下拉框选 id |
