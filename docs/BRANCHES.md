...
| 日期 | 优化内容 | 提交 / 关键 |
|------|----------|-------------|
| 2026-09-01 | **增量复制后自动归纳新标题**：`enrich_after_copy` 增加 `display_title_rename`（只改 TARGET）；与贴元数据表一起跑。存量仍用 `rename_target_display_titles`。开关 `ENABLE_DISPLAY_TITLE_RENAME`（默认 true） | `2a2a3a0` · `enrichment/steps.py` · `main.py` |
